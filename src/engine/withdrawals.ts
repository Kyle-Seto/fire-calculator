...
import { calculateAnnualExpenses, calculatePortfolioTotal } from "@/engine/fire";
 ...
 */
export function generateWithdrawalPlan(persona: Persona, years: number = 30): WithdrawalPlan[] {
	...
	for (let i = 0; i < years; i++) {
		...
		taxOwed += calculateTaxOnWithdrawal(
			nonRegW,
			"NonRegistered",
			retirementIncome + totalRrspTaxable,
		);
 ...
 */
export function calculateYieldShield(persona: Persona): YieldShieldStatus | null {
	if (persona.retirementStatus !== "retired" || persona.portfolioYield == null) {
		return null;
	}
	...
	return {
		...
		annualExpenses,
		gapAmount,
		cashCushionYears: Number.isFinite(cashCushionYears) ? cashCushionYears : Infinity,
		isFullyCovered,
	};
}
