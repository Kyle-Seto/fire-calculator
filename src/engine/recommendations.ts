...
function ruleRRSPOptimization(persona: Persona): Recommendation | null {
	...
	const { contribution, taxSavings } = calculateOptimalRRSPContribution(persona.annualIncome);
...
function ruleLowSavingsRate(persona: Persona, results: FireResults): Recommendation | null {
	if (isRetired(persona)) return null;
	if (results.savingsRate === null || results.savingsRate >= 20) return null;

	const targetRate = Math.min(results.savingsRate + 10, 30);
	const currentAnnualSavings = persona.annualIncome - results.annualExpenses;
	...
	return {
		...
		impact:
			yearsSaved > 0
				? `${yearsSaved.toFixed(1)} fewer years to FIRE`
...
function ruleCashCushionNearFI(persona: Persona, results: FireResults): Recommendation | null {
	if (isRetired(persona)) return null;
...
function ruleYieldShieldRetired(persona: Persona, results: FireResults): Recommendation | null {
	if (!isRetired(persona)) return null;
...
function ruleNonRegisteredTaxEfficiency(persona: Persona): Recommendation | null {
	if (isRetired(persona)) return null;
...
function ruleReduceSpendingImpact(persona: Persona, results: FireResults): Recommendation | null {
	if (isRetired(persona)) return null;
...
export function generateRecommendations(persona: Persona, results: FireResults): Recommendation[] {
