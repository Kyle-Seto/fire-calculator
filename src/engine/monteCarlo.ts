 ...
 */
function generateReturn(rand: () => number, meanReturn: number, stdDevReturn: number): number {
...
export function runMonteCarloSimulation(params: MonteCarloParams): MonteCarloResults {
	...
	const portfoliosByYear: number[][] = Array.from({ length: years }, () => new Array(runs));
	...
	for (let run = 0; run < runs; run++) {
		...
		if (mode === "withdrawal") {
			...
			const survived = portfoliosByYear.every((yearValues) => yearValues[run] > 0);
