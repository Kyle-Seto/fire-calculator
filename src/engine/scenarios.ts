  ...
  "mr-retiring-with-debt": () => [
    {
      ...
      apply: (p) => {
        ...
        c.lifeEvents = c.lifeEvents?.filter((e) => e.id !== "rwd-investment-loan");
        return c;
      },
    },
    {
  ...
  "mr-ivf-fire": () => [
    ...
    },
    {
      ...
      apply: (p) => {
        ...
        c.liabilities.push({ id: "sc-mortgage", label: "Mortgage", balance: 480_000 });
  ...
  if (totalDebt > 0) {
    scenarios.push({
      ...
      apply: (p) => {
        ...
        if (debt > 0) { deductFromPortfolio(c, debt); c.liabilities = []; }
        return c;
      },
    });
  }
  ...
  if (hasHome) {
    scenarios.push({
      ...
      apply: (p) => {
        ...
        const hv = ha?.value ?? 0;
        const mi = c.liabilities.findIndex((l) =>
...
function generateScenarios(persona: Persona): Scenario[] {
...
function toSnapshot(r: Omit<import("@/types").FireResults, "monteCarloResults">, income: number): MetricSnapshot {
  return {
